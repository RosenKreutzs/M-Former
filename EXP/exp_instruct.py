...
class Exp_Instruct(Trainer):
    ...
    def custom_compute_metrics(self, eval_pred: EvalLoopOutput) -> Dict[str, Any]:
        ...
        if len(stage1_indices) >= 1:
            ...
            stage1_predictions = [eval_pred.predictions[i] for i in stage1_indices]
            stage1_metrics = closed_question_metrics(stage1_predictions,
                                                     stage1_labels, self.special_id)

        # 筛选 stage 为 2 的样本（开放式任务）
        ...
        if len(stage2_indices) >= 1:
            ...
            stage2_metrics = open_question_metrics([eval_pred.predictions[i] for i in stage2_indices],
